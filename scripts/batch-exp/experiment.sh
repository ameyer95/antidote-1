...
# second argument is the time limit in seconds
# third argument is the initcommands_* file, whose directory
# we will use to store all of the subsequent experiment .jsonl files
# (Note that we convert the argument to an absolute path.)
MEMLIMIT=$1
TIMELIMIT=$2
INITFILE=$(readlink -m $3)
PATHTOFILE=${INITFILE%\/*}

cd $(dirname "$0") # Descend into the same directory as the script

NEXT=1
./next_step.sh $MEMLIMIT $TIMELIMIT $INITFILE $PATHTOFILE/$NEXT.jsonl $PATHTOFILE
while true
do
    PREV=$NEXT
    NEXT=$[$NEXT + 1]
    ...
    if [ -s $PREVFILE ]
    then
        ./next_step.sh $MEMLIMIT $TIMELIMIT $PREVFILE $NEXTFILE $PATHTOFILE
    else
        break
    fi
done
