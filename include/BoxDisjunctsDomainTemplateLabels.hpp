#ifndef BOXDISJUNCTSDOMAINTEMPLATELABELS_HPP
#define BOXDISJUNCTSDOMAINTEMPLATELABELS_HPP
...
struct BoxDisjunctsTypesLabels {
    typedef BoxStateDomainTemplateLabels<T,P,D> SingleDomain;
    typedef BoxStateAbstractionLabels<T,P,D> Single;
    typedef std::vector<Single> Many;
};


template <typename T, typename P, typename D>
class BoxDisjunctsDomainTemplateLabels : public StateDomainTemplate<typename BoxDisjunctsTypesLabels<T,P,D>::Many> {
public: typedef BoxDisjunctsTypesLabels<T,P,D> Types;
...
public:
    ...
    BoxDisjunctsDomainTemplateLabels(const typename Types::SingleDomain *box_domain) { this->box_domain = box_domain; }
...
template <typename T, typename P, typename D>
inline typename BoxDisjunctsTypesLabels<T,P,D>::Many BoxDisjunctsDomainTemplateLabels<T,P,D>::transformEachDisjunct(const typename Types::Many &element, typename Types::Single (Types::SingleDomain::*fptr)(const typename Types::Single&) const) const {
...
template <typename T, typename P, typename D>
inline typename BoxDisjunctsTypesLabels<T,P,D>::Many BoxDisjunctsDomainTemplateLabels<T,P,D>::transformEachDisjunct(const typename Types::Many &element, typename Types::Single (Types::SingleDomain::*fptr)(const typename Types::Single&, const FeatureVector&) const, const FeatureVector &x) const {
    typename Types::Many ret;
    for(auto i = element.cbegin(); i != element.cend(); i++) {
...
template <typename T, typename P, typename D>
typename BoxDisjunctsTypesLabels<T,P,D>::Many BoxDisjunctsDomainTemplateLabels<T,P,D>::filterAndUnion(const typename Types::Many &element, bool negated) const {
    typename Types::Many ret;
    // For each disjunct
    for(auto i = element.cbegin(); i != element.cend(); i++) {
...
template <typename T, typename P, typename D>
bool BoxDisjunctsDomainTemplateLabels<T,P,D>::isBottomElement(const typename Types::Many &element) const {
    return element.size() == 0; // XXX could/should check that each disjunct is non-bot
}

template <typename T, typename P, typename D>
typename BoxDisjunctsTypesLabels<T,P,D>::Many BoxDisjunctsDomainTemplateLabels<T,P,D>::binary_join(const typename Types::Many &e1, const typename Types::Many &e2) const {
...
template <typename T, typename P, typename D>
typename BoxDisjunctsTypesLabels<T,P,D>::Many BoxDisjunctsDomainTemplateLabels<T,P,D>::join(const std::vector<typename Types::Many> &elements) const {
    typename Types::Many ret;
