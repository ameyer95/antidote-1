#ifndef BOXBOUNDEDDISJUNCTSDOMAINTEMPLATEADDITION_HPP
#define BOXBOUNDEDDISJUNCTSDOMAINTEMPLATEADDITION_HPP
...
class BoxBoundedDisjunctsDomainTemplateAddition : public StateDomainTemplate<typename BoxDisjunctsTypesAddition<T,P,D>::Many> {
public:
    typedef BoxDisjunctsTypesAddition<T,P,D> Types;
...
public:
    const BoxDisjunctsDomainTemplateAddition<T,P,D> *disjuncts_domain;
    // We have access to the underlying box domain through disjuncts_domain->box_domain

    BoxBoundedDisjunctsDomainTemplateAddition(const BoxDisjunctsDomainTemplateAddition<T,P,D> *disjuncts_domain);
    BoxBoundedDisjunctsDomainTemplateAddition(const BoxDisjunctsDomainTemplateAddition<T,P,D> *disjuncts_domain, unsigned int max_num_disjuncts, const DisjunctsMergeMode &merge_mode);
...
template <typename T, typename P, typename D, typename S>
typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::combined(const typename Types::Many &element) const {
...
template <typename T, typename P, typename D, typename S>
void BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::initializeMerging(ScoreQueue &score_queue, std::set<const typename Types::Single *> &included, std::queue<const typename Types::Single *> &pending, const typename Types::Many &disjuncts) const {
...
template <typename T, typename P, typename D, typename S>
typename BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::ScoreTuple BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::selectMerge(ScoreQueue &score_queue, const std::set<const typename Types::Single *> &included) const {
    ScoreTuple ret;
    do {
        ret = score_queue.top();
        score_queue.pop();
...
template <typename T, typename P, typename D, typename S>
void BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::performMerge(const ScoreTuple &to_merge, ScoreQueue &score_queue, std::set<const typename Types::Single *> &included, std::forward_list<typename Types::Single> &new_disjuncts) const {
...
template <typename T, typename P, typename D, typename S>
void BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::prepareNextGreedyStep(ScoreQueue &score_queue, std::set<const typename Types::Single *> &included, std::queue<const typename Types::Single *> &pending) const {
...
template <typename T, typename P, typename D, typename S>
inline BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::BoxBoundedDisjunctsDomainTemplateAddition(const BoxDisjunctsDomainTemplateAddition<T,P,D> *disjuncts_domain) {
...
template <typename T, typename P, typename D, typename S>
inline BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::BoxBoundedDisjunctsDomainTemplateAddition(const BoxDisjunctsDomainTemplateAddition<T,P,D> *disjuncts_domain, unsigned int max_num_disjuncts, const DisjunctsMergeMode &merge_mode) {
    this->disjuncts_domain = disjuncts_domain;
...
template <typename T, typename P, typename D, typename S>
inline void BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::setMergeDetails(unsigned int max_num_disjuncts, const DisjunctsMergeMode &merge_mode) {
    this->max_num_disjuncts = max_num_disjuncts;
    this->merge_mode = merge_mode;
}
...
template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetImpurityEqualsZero(const typename Types::Many &element) const {
    return disjuncts_domain->meetImpurityEqualsZero(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetImpurityNotEqualsZero(const typename Types::Many &element) const {
    return disjuncts_domain->meetImpurityNotEqualsZero(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetPhiIsBottom(const typename Types::Many &element) const {
    return disjuncts_domain->meetPhiIsBottom(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetPhiIsNotBottom(const typename Types::Many &element) const {
    return disjuncts_domain->meetPhiIsNotBottom(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetXModelsPhi(const typename Types::Many &element, const FeatureVector &x) const {
    return disjuncts_domain->meetXModelsPhi(element, x);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::meetXNotModelsPhi(const typename Types::Many &element, const FeatureVector &x) const {
    return disjuncts_domain->meetXNotModelsPhi(element, x);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::applyBestSplit(const typename Types::Many &element) const {
    return disjuncts_domain->applyBestSplit(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::applySummary(const typename Types::Many &element) const {
    return disjuncts_domain->applySummary(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::applyFilter(const typename Types::Many &element) const {
    return combined(disjuncts_domain->applyFilter(element));
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::applyFilterNegated(const typename Types::Many &element) const {
...
template <typename T, typename P, typename D, typename S>
inline bool BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::isBottomElement(const typename Types::Many &element) const {
    return disjuncts_domain->isBottomElement(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::binary_join(const typename Types::Many &e1, const typename Types::Many &e2) const {
    return combined(disjuncts_domain->binary_join(e1, e2));
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypesAddition<T,P,D>::Many BoxBoundedDisjunctsDomainTemplateAddition<T,P,D,S>::join(const std::vector<typename Types::Many> &elements) const {
