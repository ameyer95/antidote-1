#ifndef BOXBOUNDEDDISJUNCTSDOMAINTEMPLATE_HPP
#define BOXBOUNDEDDISJUNCTSDOMAINTEMPLATE_HPP
...
template <typename T, typename P, typename D, typename S>
typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::combined(const typename Types::Many &element) const {
...
template <typename T, typename P, typename D, typename S>
typename BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::ScoreTuple BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::selectMerge(ScoreQueue &score_queue, const std::set<const typename Types::Single *> &included) const {
    ScoreTuple ret;
    do {
        ret = score_queue.top();
        score_queue.pop();
...
template <typename T, typename P, typename D, typename S>
void BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::performMerge(const ScoreTuple &to_merge, ScoreQueue &score_queue, std::set<const typename Types::Single *> &included, std::forward_list<typename Types::Single> &new_disjuncts) const {
...
template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetImpurityEqualsZero(const typename Types::Many &element) const {
    return disjuncts_domain->meetImpurityEqualsZero(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetImpurityNotEqualsZero(const typename Types::Many &element) const {
    return disjuncts_domain->meetImpurityNotEqualsZero(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetPhiIsBottom(const typename Types::Many &element) const {
    return disjuncts_domain->meetPhiIsBottom(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetPhiIsNotBottom(const typename Types::Many &element) const {
    return disjuncts_domain->meetPhiIsNotBottom(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetXModelsPhi(const typename Types::Many &element, const FeatureVector &x) const {
    return disjuncts_domain->meetXModelsPhi(element, x);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::meetXNotModelsPhi(const typename Types::Many &element, const FeatureVector &x) const {
    return disjuncts_domain->meetXNotModelsPhi(element, x);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::applyBestSplit(const typename Types::Many &element) const {
    return disjuncts_domain->applyBestSplit(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::applySummary(const typename Types::Many &element) const {
    return disjuncts_domain->applySummary(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::applyFilter(const typename Types::Many &element) const {
    return combined(disjuncts_domain->applyFilter(element));
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::applyFilterNegated(const typename Types::Many &element) const {
...
template <typename T, typename P, typename D, typename S>
inline bool BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::isBottomElement(const typename Types::Many &element) const {
    return disjuncts_domain->isBottomElement(element);
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::binary_join(const typename Types::Many &e1, const typename Types::Many &e2) const {
    return combined(disjuncts_domain->binary_join(e1, e2));
}

template <typename T, typename P, typename D, typename S>
inline typename BoxDisjunctsTypes<T,P,D>::Many BoxBoundedDisjunctsDomainTemplate<T,P,D,S>::join(const std::vector<typename Types::Many> &elements) const {
